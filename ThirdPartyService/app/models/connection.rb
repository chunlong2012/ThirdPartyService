...
class Connection
	...
	def reconnect
		...
		ApnsPushLog.log( "Connect to APNS with (#{ @pem }) pem file" )
	end
	...
	def sendmsg_sub( msg )
		begin
			...
			ApnsPushLog.log( "Error: push send connection error, try to reconnect." )
			return false 
		end
	end
	...
	def sendmsg( msg )
		for i in 1 .. 3 
			return if sendmsg_sub( msg )
			reconnect
		end
		raise "3 reconnection fail, system down."
	end
